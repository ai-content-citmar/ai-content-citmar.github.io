...
function mountAIGeneratorMain(){
  ...
  function renderSettingsPage(){
      ...
      <div class="panel settings-page">
        ...
        <div class="control-pane">
          ...
          <div style="display:flex;gap:10px;align-items:center;margin-top:12px;padding-top:12px;border-top:1px solid var(--border-color)">
            ...
            <button id="settingsThemeToggle" class="theme-settings-btn secondary" aria-label="Toggle dark/light mode">
    ...
    delBtn?.addEventListener('click', async ()=>{
      ...
      if(!backendURL) return showStatus('Backend URL tidak dikonfigurasi', 'error')
      // Gunakan async deleteWithConfirm dengan fallback ke confirm native
      (async ()=>{
        try{
          let confirmed = false
          ...
          const res = await fetch(`${backendURL}/ai/delete-key?provider=${encodeURIComponent(provider)}`, { 
            method: 'DELETE',
            headers: getAuthHeaders()
          })
          const j = await res.json().catch(()=>({}))
          if(res.ok && j?.ok){
            ...
            throw new Error(j?.error || 'Penghapusan gagal')
          }
        }catch(e){ 
          console.warn('[settings.deleteKey] error:', e)
  ...
  function renderPresetsPage(){
    ...
    placeholder.innerHTML = `
      <div class="panel presets-page">
        ...
        <div id="presetsList" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(360px,1fr));gap:12px;margin-top:10px"></div>
    ...
    function openEditor(key){
      ...
      document.getElementById('cancelPresetEd').addEventListener('click', ()=>{ try{ overlay.remove() }catch(e){}; renderList() })
      ...
      card.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px">
            ...
            <button data-copy-audio="${idSafe}" style="padding:6px;border-radius:6px">Copy Audio Info</button>
          </div>
        </div>
        ...
        <div style="margin-top:8px;font-size:12px;color:#b0b0b0">Audio Recommendation</div>
        <div id="${idSafe}_audio" style="margin-top:4px;color:#fff;white-space:pre-line">${''}</div>
      `
